...
namespace latestratesMicroservice
{
    /// <summary>
    ...
    public class Function : ICloudEventFunction<MessagePublishedData>
    {
            ...
            {
                _logger.LogInformation("Accessed Latest rate Cloud Function");
                ...
                string symbols = Obj.Symbols.ToString();// to be added to the urls


                RestClient client = new RestClient("https://microservicesix-rlz3wkvhza-uc.a.run.app/api/v1/TaskSix/getexchangerate/EUR/"+symbols);//add twitter microservice here
                RestRequest request = new RestRequest();

                request.Method = Method.Get;
                var response = client.ExecuteAsync(request);
                _logger.LogInformation("Request to Latest Exchange rate MicroService Has been Sent");

                response.Wait();

                _logger.LogInformation("Obtained Response In Latest Rates CF : "+response.Result.Content);

                dynamic parsedReponse = JsonConvert.DeserializeObject(response.Result.Content);

                ExchangeRateModel exchangeRateModel = JsonConvert.DeserializeObject<ExchangeRateModel>(parsedReponse);

                _logger.LogInformation(" Parsed Json in Latest rates CF");
                ...
                docRef.SetAsync(exchangeRateModel).Wait();
                _logger.LogInformation("Json Saved to Database in latest rates CF");
